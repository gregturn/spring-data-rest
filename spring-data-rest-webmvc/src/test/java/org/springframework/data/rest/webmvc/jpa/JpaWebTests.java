...
public class JpaWebTests extends CommonWebTests {
	...
	public void accessPersons() throws Exception {
		...
		Link nextLink = linkTestUtils.assertHasLinkWithRel(Link.REL_NEXT, response);
		assertDoesNotHaveLinkWithRel(Link.REL_PREVIOUS, response);

		response = webTestUtils.request(nextLink);
		linkTestUtils.assertHasLinkWithRel(Link.REL_PREVIOUS, response);
		nextLink = linkTestUtils.assertHasLinkWithRel(Link.REL_NEXT, response);
	...
	public void exposesLinkForRelatedResource() throws Exception {
		...
		Link ordersLink = linkTestUtils.assertHasLinkWithRel("orders", response);
	...
	public void createThenPatch() throws Exception {
		...
		Link bilboLink = linkTestUtils.assertHasLinkWithRel("self", bilbo);
	...
	public void putShouldWorkDespiteExistingLinks() throws Exception {
		...
		Link frodoLink = linkTestUtils.assertHasLinkWithRel("self", createdPerson);
		...
		linkTestUtils.assertHasLinkWithRel("self", overwrittenResponse);
	...
	private List<Link> preparePersonResources(Person primary, Person... persons) throws Exception {
		...
		MockHttpServletResponse primaryResponse = postAndGet(peopleLink, mapper.writeValueAsString(primary),
				MediaType.APPLICATION_JSON);
		links.add(linkTestUtils.assertHasLinkWithRel("siblings", primaryResponse));
		...
		for (Person person : persons) {
			...
			links.add(linkTestUtils.assertHasLinkWithRel(Link.REL_SELF, response));
		}
	...
	public void doesNotAllowGetToCollectionResourceIfFindAllIsNotExported() throws Exception {
		...
		Link link = linkTestUtils.discoverUnique("addresses");
	...
	public void returnsProjectionIfRequested() throws Exception {
		...
		Link orders = linkTestUtils.discoverUnique("orders");
	...
	public void onlyLinksShouldAppearWhenExecuteSearchCompact() throws Exception {
		...
		Link daenerysLink = linkTestUtils.assertHasLinkWithRel("self", createdPerson);
		assertJsonPathEquals("$.firstName", "Daenerys", createdPerson);

		Link searchLink = linkTestUtils.discoverUnique(peopleLink, "search");
		Link byFirstNameLink = linkTestUtils.discoverUnique(searchLink, "findFirstPersonByFirstName");
	...
	public void rendersExcerptProjectionsCorrectly() throws Exception {
		...
		Link authorsLink = linkTestUtils.discoverUnique("authors");
		...
		linkTestUtils.follow(new Link(href)).andExpect(linkTestUtils.hasLinkWithRel("books"));
	}
	...
	private void assertPersonWithNameAndSiblingLink(String name) throws Exception {
		...
		MockHttpServletResponse response = webTestUtils.request(linkTestUtils.discoverUnique("people"));
